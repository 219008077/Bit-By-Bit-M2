...
namespace Project
{
    public partial class Dashboard : Form
    {
        ...
        private void AddCustomerButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Missing customer Last name", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (string.IsNullOrWhiteSpace(email))
            {
                MessageBox.Show("Missing customer email", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (string.IsNullOrWhiteSpace(haddress))
            {
                MessageBox.Show("Missing customer home address", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (string.IsNullOrWhiteSpace(phone))
            {
                MessageBox.Show("Missing customer phone number", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            //
        ...
        private void AddServiceBtn_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(itemID, out id) && int.TryParse(TicketID, out tId) && decimal.TryParse(AddServicePrice.SelectedValue.ToString(), out amount))
            {
                ...
                {
                    MessageBox.Show($"service error-{ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                }
            }
            else
            {
        ...
        private void payTicketDataGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            CollectionType = payTicketDataGrid.SelectedRows[0].Cells[6].Value.ToString();
        ...
        private void receiptBtn_Click(object sender, EventArgs e)
        {
            if (TicketID > 0)
            {
                Form receipt = new Receipt(TicketID, CollectionType);
                receipt.ShowDialog();
            }
        }
        ...
        {
            dashboardTabs.SelectTab(1);
        }

        private void viewCustomers_Click(object sender, EventArgs e)
        {
            dashboardTabs.SelectTab(0);
        }

        private void viewPayments_Click(object sender, EventArgs e)
        {
            dashboardTabs.SelectTab(2);
        }

        private void backItems_Click(object sender, EventArgs e)
        {
            dashboardTabs.SelectTab(1);
        }
    }
